...
import { getPageData, updateAbout } from '@/app/lib/data';
...
export default function AboutForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!result) return;
      setData(result);
      result.imgSrc !== null && setImgSrc(result?.imgSrc);
    };
    fetchData();
  }, []);

  const updateAboutWithId = data?.id ? updateAbout.bind(null, data.id) : null;

  const [state, formAction] = useFormState(
    updateAboutWithId ??
      (() => Promise.resolve({ message: 'No data found', errors: {} })),
    {
      message: '',
      errors: {},
    },
