  ...
  return (
    <section className="my-4 grid w-full gap-2">
      <div className="grid-cols grid w-full grid-cols-12 justify-between lg:gap-x-3">
        ...
        <span className="col-span-4 text-xs italic text-secondary lg:col-span-3 lg:text-sm">
          {experience.title}
        </span>
        <span className="text-lightGray col-span-6 text-xs font-semibold lg:col-span-3 lg:text-sm">
          {experience.organization}
        </span>
        <span className="col-span-0 hidden text-xs text-primary lg:col-span-2 lg:block lg:text-sm">
          {experience.location}
        </span>
        <span className="col-span-2 text-right text-xs text-primary lg:text-sm">
          {experience.startDate}
          {experience.endDate && ` - ${experience.endDate}`}
        </span>
      </div>
      <div className="ml-5 hidden w-full flex-col gap-1 text-sm font-light text-primary lg:flex">
