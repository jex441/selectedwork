...
export async function POST(req: Request, res: NextApiResponse) {
  ...
  const session =
    ...
    (await stripe.checkout.sessions.create({
      ...
      success_url: `http${process.env.NODE_ENV === 'production' ? 's' : ''}://app.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/account`,
      cancel_url: `http${process.env.NODE_ENV === 'production' ? 's' : ''}://app.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/account`,
    }));
